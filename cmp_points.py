...
# ^^^ from http://jorisvr.nl/files/graphmatching/20130407/mwmatching.py
...
def match(lp, rp):
    ...
    #     for j, p2 in enumerate(rp):
    #         distances.append((distance(p, p2), i, j))
    # distances.sort()
    # pairs = []
    # for d, i, j in distances:
    #     if i not in i_done and j not in j_done:
    #         pairs.append((d, lp[i], rp[j]))
    #         i_done.add(i)
    #         j_done.add(j)
    # for t in pairs:
    #     print t
    ...
    #     [
    #         [-distance(p1, p2) for p2 in rp]
    #         for p1 in lp
    #     ]
    # )
    pairs = maxWeightMatching([(i, len(lp) + j, -distance(p1, p2)) for i, p1 in enumerate(lp) for j, p2 in enumerate(rp)], True)
    matches = []
    distance_pairs = []
    for i, j in enumerate(pairs):
        if i >= len(lp):
            break
        matches.append(j - len(lp))
        if j == -1:
            continue
        j -= len(lp)
        distance_pairs.append((distance(lp[i], rp[j]), lp[i], rp[j], i, j))

    return (distance_pairs, matches)

def to_points(l):
    ps = []
    for i in range(len(l) / 3):
        ps.append((l[i * 3], l[i * 3 + 1], l[i * 3 + 2]))
    return ps
